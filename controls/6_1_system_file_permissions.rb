...
control 'cis-dil-benchmark-6.1.10' do
  ...
  describe command("df --local -P | awk '{ if (NR!=1) print $6 }' | xargs -I '{}' find '{}' -xdev -type f -perm -0002 -not -path '/var/lib/docker/overlay2/*'") do
...
control 'cis-dil-benchmark-6.1.12' do
  ...
  describe command("df --local -P | awk '{ if (NR!=1) print $6 }' | xargs -I '{}' find '{}' -xdev -nogroup -not -path '/var/lib/docker/overlay2/*'") do
    its('stdout') { should cmp '' }
  end
end
